...
const NewMediaInput = ({ climberId, handleChange }: MediaInputProps) => {
    ...
    const [mediaClimbIds, setMediaClimbIds] = useState<Number[]>([])

    const {loading, error, data} = useQuery(QUERY_ALL_CLIMBS)

    if (loading) return <p>Loading...</p>
    if (error) return <p>Error : {error.message}</p>

    const climbsList = data.climb.map((climb: ClimbProps) => {
        return {
            key: climb.id,
            value: climb.id,
            label: climb.name
    ...
    return (
        ...
        <Dropdown
            ...
            placeholder="Climb?"
            onChange={value => {
                setMediaClimbIds([...mediaClimbIds, Number(value)])
            }}/>
        ...
        <button
            ...
            onClick={() => {
                ...
                const variables = {
                    ...
                    climbMedia: mediaClimbIds.map(id => {
                        return {
                            media: {
                                data: {
                                    label: mediaLabelInput,
                ...
                .then(rs => {
                    const affectedRows = rs.data?.insert_climber_media.affected_rows + rs.data?.insert_climb_media.affected_rows
